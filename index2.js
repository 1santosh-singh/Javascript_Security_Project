...
app.get('/', function (req, resp){  //retrieve
...
app.get('/mul/:n/:m',  (req, resp)=>{
    var n = Number(req.params.n);
    var m = Number(req.params.m);
    resp.send(` of ${n} is: ${n*m}`);
});
app.get('/addition/:a/:b', function (req, resp){
    var a = Number(req.params.a);
    var b = Number(req.params.b);
    var c = a+b;
    resp.send(`Addition of ${a} and ${b} is: ${c}`);
});

app.listen(9000, () => console.log("Application server is running"))